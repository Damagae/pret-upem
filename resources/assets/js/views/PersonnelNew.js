...
export default class PersonnelNew extends Component {
    /*
          ...
          <div className="row">
            ...
            <div className="col">
              <div className="form-group">
                ...
                <input onBlur={() => { this.checkFormField("participants", ["required", "alpha"]) }} onChange={() => { this.checkFormField("participants", ["required", "alpha"]) }} onPaste={() => { this.checkFormField("participants", ["required", "alpha"]) }} onInput={() => { this.checkFormField("participants", ["required", "alpha"]) }} type="text" className="form-control" id="participants" placeholder="Noms et prénoms des étudiants" />
              </div>
            </div>
          </div>
    ...
    infopersonnel() {
      return (
        <div key='inforpersonnel' className={'infopersonnel ' + (this.isVisible('infopersonnel') ? 'visible' : 'invisible')}>
          ...
          <div className="row">
            ...
            <div className="col">
              <div className="form-group">
                ...
                <input onBlur={() => { this.checkFormField("carte", ["required", "carte"]) }} onChange={() => { this.checkFormField("carte", ["required", "carte"]) }} onPaste={() => { this.checkFormField("carte", ["required", "carte"]) }} onInput={() => { this.checkFormField("carte", ["required", "carte"]) }} type="text" className="form-control" id="carte" placeholder="12345678" />
              </div>
              <div className="form-group">
                <label htmlFor="nom-assurance">Nom de l'assurance</label>
                <input onBlur={() => { this.checkFormField("nom-assurance", ["required", "alphaNum"]) }} onChange={() => { this.checkFormField("nom-assurance", ["required", "alphaNum"]) }} onPaste={() => { this.checkFormField("nom-assurance", ["required", "alphaNum"]) }} onInput={() => { this.checkFormField("nom-assurance", ["required", "alphaNum"]) }} type="text" className="form-control" id="nom-assurance" placeholder="12345678" />
              </div>
              <div className="form-group">
                <label htmlFor="num-assurance">Numéro de police d'assurance</label>
                <input onBlur={() => { this.checkFormField("num-assurance", ["required", "numeric"]) }} onChange={() => { this.checkFormField("num-assurance", ["required", "numeric"]) }} onPaste={() => { this.checkFormField("num-assurance", ["required", "numeric"]) }} onInput={() => { this.checkFormField("num-assurance", ["required", "numeric"]) }} type="text" className="form-control" id="num-assurance" placeholder="12345678" />
              </div>
              <div className="form-group">
                <label htmlFor="telephone">Numéro de téléphone portable</label>
                <input onBlur={() => { this.checkFormField("telephone", ["required", "telephone"]) }} onChange={() => { this.checkFormField("telephone", ["required", "telephone"]) }} onPaste={() => { this.checkFormField("telephone", ["required", "telephone"]) }} onInput={() => { this.checkFormField("telephone", ["required", "telephone"]) }} type="text" className="form-control" id="telephone" placeholder="Téléphone" />
              </div>
            </div>
          </div>
          <button className='btn btn-orange-outlined' onClick={() => { this.previous() }}>Précédent</button>
    ...
    validation() {
      ...
      return (
        <div key='validation' className={'validation ' + (this.isVisible('validation') ? 'visible' : 'invisible')}>
          <h2>Validation</h2>
            ...
            <div className="row align-items-center">
              <div className="col validation-section" onMouseLeave={() => { this.hideIcon('edit1')}} onMouseOver={() => { this.showIcon('edit1')}} onClick={() => { this.updateActiveFormSection('infoprojet') }}>
                ...
                <p><b>Bureau de prêt : </b> { this.getInputValue('bureau') ? this.getInputValue('bureau') : warning } </p>
                <p><b>Date de début de l'emprunt : </b> { this.getInputValue('date-debut') ? this.getInputValue('date-debut') : warning } </p>
                <p><b>Date de fin de l'emprunt : </b> { this.getInputValue('date-fin') ? this.getInputValue('date-fin') : warning } </p>
          ...
          <button className='btn btn-orange-outlined' onClick={() => { this.previous() }}>Précédent</button>
          <button className='btn btn-orange' onClick={() => { this.send() }} disabled={ !this.validate() ? 'disabled' : '' }>Valider la demande</button>
          <p className="policy">En validant, vous acceptez la <a target="_blank" href="/personnel/convention-pret-audiovisuel-upem">convention de prêt audiovisuel de l'UPEM</a>.</p>
    ...
    send() {
      ...
      data.append('emprunteur_mail', 'etudiant@etud.u-pem.fr')  // Email à retrouver grâce au ldap !!!!!!!!!!!!
      ...
      if (this.validate()) {
        axios.post('/api/c/personnel/new', // Envoie les données de la convention
          data
        )
        .then(response => {
          const id = response.data.id
          materiel.map(e => {
                axios.post('/api/m/new', { // Envoie les matériels en utilisant l'id de la convention qui vient d'être créée
                  convention_id: id,
                  nom: e.nom,
                  quantite: e.qte
                })
                .then(response => {
                  ...
                  document.location = ("/personnel/confirmation")
                })
...
if (document.getElementById('personnel-new')) {
    ReactDOM.render(<PersonnelNew />, document.getElementById('personnel-new'));
}
